...
function formatFailedAttemptDetail(attemptNumber) {
    return `Invalid credentials (attempt ${attemptNumber}/${MAX_FAILED_ATTEMPTS})`;
}
function formatLockoutDetail(attemptNumber) {
    return `Account locked (attempt ${attemptNumber}/${MAX_FAILED_ATTEMPTS})`;
}
function registerFailedAttempt(record, now) {
...
function formatLogEntry(timestamp, username, eventType, detail) {
...
function log_event(username, eventType, detail) {
...
async function loadActivityLogFromDb() {
    ...
    data?.forEach((row) => {
        ...
        const entry = formatLogEntry(timestamp, row.username ?? 'unknown', row.event_type, row.detail ?? '');
        appendLogEntry(entry);
    });
}
function getActivityLogTail(limit) {
...
function issueSession(res, user) {
    const signOptions = { expiresIn: config.jwtExpiresIn };
...
app.get('/api/auth/activity', requireUser, async (req, res) => {
